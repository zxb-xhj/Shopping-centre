...
import com.xhj.search.vo.AttrResponseVo;
...
public class MallSearchServiceIpml implements MallSearchService {
    ...
    public SearchResult search(SearchParam param) {
        ...
        try {
            // 2、执行检索请求
            SearchResponse response = highLevelClient.search(searchRequest, XhjElasticSearchConfig.COMMON_OPTIONS);
    ...
    private SearchResult buildSearchResult(SearchResponse response,SearchParam param) {
        ...
        if (param.getAttrs() != null && param.getAttrs().size() > 0) {
            List<SearchResult.NavVo> collect = param.getAttrs().stream().map(attr -> {
                ...
                navVo.setLink("http://search.xhj.com/list.html?" + replace);
