  ...
  <el-dialog :title="!dataForm.id ? '新增' : '修改'" :close-on-click-modal="false" :visible.sync="visible"
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" label-width="120px">
      ...
      <el-form-item label="所属分类" prop="catelogId">
        <category-cascader @getcascader="getchange" ref="categoryCascader"></category-cascader>
      </el-form-item>
...
export default {
  ...
  methods: {
    getchange(data, datas) {
      this.dataForm.catelogId = data;
      this.catelogPath = datas;
      console.log(this.dataForm.catelogId);
      if (data != undefined) {
        this.$http({
          url: this.$http.adornUrl(`/product/attrgroup/listId/${data}`),
          method: 'get',
          params: this.$http.adornParams({})
        }).then(({ data }) => {
          console.log(data);
          this.attrGroups = data.attrGroup;
        })
      }
    },
    init(id) {
          ...
          this.$http({
            ...
            if (data && data.code === 0) {
              ...
              this.$nextTick(() => {
                ...
                this.$refs.categoryCascader.getpath(data.attr.catelogPath);
              });
            }
          });
        }
    ...
    dialogClose() {
      this.catelogPath = [];
      this.$nextTick(() => {
        this.$refs.categoryCascader.getpathnull();
      });
    }
  }
};
